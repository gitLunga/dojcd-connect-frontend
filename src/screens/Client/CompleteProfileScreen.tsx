// screens/Client/CompleteProfileScreen.tsx
// Updated to match web ProfileScreen:
//  - Added "Account Information" card at the top (matches web combined profile view)
//  - All original RN logic preserved exactly:
//    camera via expo-image-picker, gallery, PDF via expo-document-picker,
//    DateTimePicker, AsyncStorage, authAPI.completeProfile
...
// SelectField with slide-up modal — preserved from original
    ...
    return (
        <>
            <View style={sf.wrap}>
                ...
                >
                    <Text style={[sf.btnText, !value && sf.placeholder]}>
                    ...
                    <View style={sf.sheet}>
                        ...
                        <FlatList
                            ...
                            renderItem={({ item }: any) => (
                                ...
                                >
                                    <Text style={[sf.optionText, value === item.value && sf.optionTextActive]}>
                                        {item.label}
                                    </Text>
...
const sf = StyleSheet.create({
    ...
    btn:         { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: C.bg, borderWidth: 1.5, borderColor: C.border, borderRadius: 14, paddingHorizontal: 14, paddingVertical: 14 },
    ...
    sheetHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 20, borderBottomWidth: 1, borderBottomColor: C.border },
...
export default function CompleteProfileScreen({ navigation }: any) {
    ...
    // Load user from AsyncStorage on mount — NEW: also pre-fills provider/duration if set
    useEffect(() => {
        (async () => {
            ...
            if (ud) {
                ...
                setUser(u);
                if (u.network_provider)         setNetwork(u.network_provider);
                if (u.contract_duration_months) setDuration(String(u.contract_duration_months));
            }
        })();
    }, []);

    // Document upload methods — preserved exactly from original
    const pickDoc = (key: string) => {
        // Use a simple bottom-sheet style modal instead of Alert.alert for consistency
        ImagePicker.requestCameraPermissionsAsync().then(() => { /* pre-request */ });
        // Show options via existing approach
    ...
    const takePhoto = async (key: string) => {
        ...
        if (status !== 'granted') { toast.warning('Permission Required', 'Camera permission is needed.'); return; }
        const r = await ImagePicker.launchCameraAsync({ mediaTypes: ImagePicker.MediaTypeOptions.Images, allowsEditing: true, quality: 0.8 });
        if (!r.canceled && r.assets[0]) {
            setDocs(p => ({ ...p, [key]: { uri: r.assets[0].uri, name: `${key}_${Date.now()}.jpg`, type: 'image/jpeg', size: 0 } }));
    ...
    const pickImage = async (key: string) => {
        ...
        if (!r.canceled && r.assets[0]) {
            ...
            setDocs(p => ({ ...p, [key]: { uri: r.assets[0].uri, name: `${key}_${Date.now()}.jpg`, type: mimeType, size: 0 } }));
            toast.success('Image selected');
        }
    };

    const pickPDF = async (key: string) => {
        const r = await DocumentPicker.getDocumentAsync({ type: ['application/pdf', 'application/msword'], copyToCacheDirectory: true });
        if (r.assets?.[0]) {
            const a = r.assets[0];
            let mimeType = a.mimeType || 'application/pdf';
            if (mimeType === 'image/jpg') mimeType = 'image/jpeg';
            setDocs(p => ({ ...p, [key]: { uri: a.uri, name: a.name, type: mimeType, size: a.size || 0 } }));
            toast.success('Document selected');
        }
    };

    const validate = () => {
        if (!network)  { toast.warning('Missing Field', 'Please select a network provider.'); return false; }
        if (!duration) { toast.warning('Missing Field', 'Please select a contract duration.'); return false; }
        for (const doc of DOCS.filter(d => d.required)) {
            if (!docs[doc.key]) { toast.warning('Missing Document', `${doc.title} is required.`); return false; }
        }
        return true;
    };
    ...
    const submit = async () => {
        ...
        try {
            const ud = await AsyncStorage.getItem('user');
            if (!ud) { toast.error('Session Expired', 'Please login again.'); navigation.goBack(); return; }
            const u = JSON.parse(ud);
            ...
            const result = await authAPI.completeProfile(u.client_user_id, data);
            if (result.success) {
                const updatedUser = { ...u, ...result.data?.user, registration_status: 'Profile_Completed' };
            ...
            <View style={s.root}>
                ...
                <View style={s.header}>
                    ...
                    <View style={s.progressRow}>
                        {[
                            { done: !!network,       label: 'Network' },
                            { done: !!duration,      label: 'Contract' },
                            { done: requiredDone,    label: 'Documents' },
                        ].map((p, i, arr) => (
                            <React.Fragment key={p.label}>
                                <View style={s.progressItem}>
                                    <Ionicons name={p.done ? 'checkmark-circle' : 'ellipse-outline'} size={16} color={p.done ? '#4ADE80' : 'rgba(255,255,255,0.4)'} />
                                    <Text style={s.progressLabel}>{p.label}</Text>
                                </View>
                                {i < arr.length - 1 && <View style={s.progressLine} />}
                            </React.Fragment>
                        ))}
                    </View>
                </View>
                ...
                <ScrollView style={s.scroll} showsVerticalScrollIndicator={false} contentContainerStyle={s.scrollContent}>
                    ...
                    {/* ── Account Information card (NEW — matches web ProfileScreen) ── */}
                    {user && (
                        <View style={s.card}>
                            <View style={s.cardTitleRow}>
                                <View style={s.cardTitleIcon}><Ionicons name="person-outline" size={18} color={C.accent} /></View>
                            ...
                            {[
                                ...
                                { label: 'Contract',         value: user.contract_duration_months ? `${user.contract_duration_months} Months` : 'Not set', icon: 'calendar-outline' },
                            ].map((row, i) => (
                                <View key={i} style={[s.infoRow, i === 3 && { borderBottomWidth: 0 }]}>
                                    <Ionicons name={row.icon as any} size={15} color={C.muted} style={{ marginRight: 10 }} />
                    ...
                    <View style={s.card}>
                        <View style={s.cardTitleRow}>
                            <View style={s.cardTitleIcon}><Ionicons name="phone-portrait-outline" size={18} color={C.accent} /></View>
                            <Text style={s.cardTitle}>Contract Details</Text>
                        </View>

                        <SelectField label="NETWORK PROVIDER" value={network} placeholder="Select your provider" options={PROVIDERS} onSelect={setNetwork} disabled={loading} />
                        <SelectField label="CONTRACT DURATION" value={duration} placeholder="Select contract length" options={DURATIONS} onSelect={setDuration} disabled={loading} />

                        <View style={{ marginBottom: 8 }}>
                            <Text style={sf.label}>CONTRACT END DATE</Text>
                            <TouchableOpacity style={sf.btn} onPress={() => setShowDatePicker(true)}>
                                <Text style={sf.btnText}>{endDate.toLocaleDateString('en-ZA', { day: 'numeric', month: 'long', year: 'numeric' })}</Text>
                            ...
                            {showDatePicker && (
                                <DateTimePicker
                                    ...
                                    mode="date"
                                    display="default"
                                    onChange={(e, d) => { setShowDatePicker(false); if (d) setEndDate(d); }}
                    ...
                    <View style={s.card}>
                        <View style={s.cardTitleRow}>
                            <View style={s.cardTitleIcon}><Ionicons name="documents-outline" size={18} color={C.accent} /></View>
                        ...
                        {DOCS.map(doc => {
                            ...
                            return (
                                <View key={doc.id} style={[s.docRow, file && s.docRowDone]}>
                                    <View style={[s.docIcon, { backgroundColor: file ? C.greenSoft : C.bg }]}>
                                    ...
                                    >
                                        <Ionicons name={file ? 'close' : 'cloud-upload-outline'} size={18} color={file ? C.rose : C.accent} />
                                    </TouchableOpacity>
                                </View>
                            );
                        })}
                    ...
                    <View style={s.submitWrap}>
                        ...
                        >
                            {loading
                                ? <><ActivityIndicator color="#fff" size="small" /><Text style={s.submitText}>Submitting…</Text></>
                                : <><Ionicons name="checkmark-circle-outline" size={20} color="#fff" /><Text style={s.submitText}>Complete Profile</Text></>
                            }
                        </TouchableOpacity>
...
const s = StyleSheet.create({
    ...
    headerRing:   { position: 'absolute', width: 240, height: 240, borderRadius: 120, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)', top: -80, right: -60 },
    ...
    headerIcon:   { width: 60, height: 60, borderRadius: 18, backgroundColor: 'rgba(255,255,255,0.1)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.15)', justifyContent: 'center', alignItems: 'center', marginBottom: 14 },
    headerTitle:  { fontSize: 24, fontWeight: '800', color: '#fff', marginBottom: 6 },
    headerSub:    { fontSize: 13, color: 'rgba(255,255,255,0.55)', textAlign: 'center', lineHeight: 19, paddingHorizontal: 20 },
    ...
    card:          { backgroundColor: C.surface, borderRadius: 20, padding: 20, marginBottom: 14, borderWidth: 1, borderColor: C.border, shadowColor: C.navy, shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.06, shadowRadius: 10, elevation: 3 },
    cardTitleRow:  { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 4 },
    cardTitleIcon: { width: 34, height: 34, borderRadius: 10, backgroundColor: C.accentSoft, justifyContent: 'center', alignItems: 'center' },
    ...
    // Account info styles (NEW — matches web)
    avatarRow:       { flexDirection: 'row', alignItems: 'center', gap: 14, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: C.border, marginBottom: 4 },
    avatar:          { width: 50, height: 50, borderRadius: 14, backgroundColor: C.accent, justifyContent: 'center', alignItems: 'center', flexShrink: 0 },
    avatarText:      { fontSize: 20, fontWeight: '900', color: '#fff' },
    accountName:     { fontSize: 15, fontWeight: '800', color: C.text, marginBottom: 2 },
    accountEmail:    { fontSize: 12, color: C.muted, marginBottom: 6 },
    ...
    infoLabel:       { fontSize: 12, color: C.muted, width: 120, flexShrink: 0 },
    infoVal:         { fontSize: 12, fontWeight: '600', color: C.text, flex: 1 },

    // Document rows
    docRow:          { flexDirection: 'row', alignItems: 'center', padding: 14, borderRadius: 14, marginBottom: 10, backgroundColor: C.bg, borderWidth: 1, borderColor: C.border },
    ...
    reqBadge:        { fontSize: 10, color: C.amber, fontWeight: '700', backgroundColor: C.amberSoft, paddingHorizontal: 7, paddingVertical: 2, borderRadius: 8 },
    docSubtitle:     { fontSize: 12, color: C.muted },
    docFileName:     { fontSize: 12, color: C.green, fontWeight: '500' },
    docActionBtn:    { width: 36, height: 36, borderRadius: 10, backgroundColor: C.accentSoft, justifyContent: 'center', alignItems: 'center' },
    docActionBtnChange: { backgroundColor: C.roseSoft },

    submitWrap:     { marginTop: 4 },
    submitBtn:      { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 10, backgroundColor: C.navy, borderRadius: 18, paddingVertical: 18, shadowColor: C.navy, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.28, shadowRadius: 12, elevation: 8, marginBottom: 14 },
