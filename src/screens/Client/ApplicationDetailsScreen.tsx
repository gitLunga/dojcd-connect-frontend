// screens/Client/ApplicationDetailsScreen.tsx
// Updated to match web version:
//   - ConfirmDialog replaces Alert.alert for cancel confirmation
//   - All original API logic, timeline, styles preserved exactly
...
export default function ApplicationDetailsScreen() {
    ...
    const load = async () => {
        try {
            ...
            setUser(u);
            const res = await deviceAPI.getApplicationDetails(u.client_user_id, applicationId);

            // Mirror web version exactly — robustly unwrap all possible nesting shapes
            const d = res?.data;
            let raw: any =
                d?.data?.application ??
                d?.data?.data        ??
            ...
            // If raw is an array (guard), take first item
            if (Array.isArray(raw)) raw = raw[0] ?? null;

            // Normalise alternate field name variants (mirrors web version exactly)
    ...
    // Mirror web version: include device name and details in cancel dialog
    const handleCancel = () => {
    ...
    // Safe date formatters — handle null like web version
    const fmtDate      = (d: string | null) => { if (!d) return '—'; return new Date(d).toLocaleDateString('en-ZA', { day: 'numeric', month: 'long',  year: 'numeric', hour: '2-digit', minute: '2-digit' }); };
    const fmtDateShort = (d: string | null) => { if (!d) return '—'; return new Date(d).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short', year: 'numeric' }); };
        ...
        return (
            <DrawerLayout>
                <View style={s.errorScreen}>
                    ...
                    <TouchableOpacity style={s.errBackBtn} onPress={() => navigation.goBack()}>
                        <Text style={s.errBackBtnText}>Go Back</Text>
                    </TouchableOpacity>
                </View>
            </DrawerLayout>
        );
    ...
    // Mirror web: derive applicant name/email with fallback to localStorage user
    ...
    // Contract total like web version
            ...
            <View style={s.root}>
                ...
                <ScrollView style={s.scroll} showsVerticalScrollIndicator={false}>
                    ...
                    <View style={[s.statusHero, { backgroundColor: meta?.bg || C.slateSoft }]}>
                        <View style={[s.statusIcoWrap, { backgroundColor: (meta?.dot ?? C.border) + '25' }]}>
                            <Ionicons name={meta?.icon || 'help-circle'} size={36} color={meta?.dot || C.muted} />
                        </View>
                        <Text style={[s.statusLabel, { color: meta?.fg || C.muted }]}>
                            {meta?.label || application.application_status}
                        </Text>
                        ...
                        {application.application_status === 'Pending' && (
                            ...
                            >
                                {cancelling
                                    ...
                                    : <>
                                        <Ionicons name="close-circle-outline" size={17} color={C.rose} />
                        ...
                        <View style={s.deviceCard}>
                            ...
                            <View style={s.planRow}>
                                ...
                                <View style={s.planPill}>
                                    ...
                                    <Text style={s.planPillText}>{application.contract_duration_months} months</Text>
                                </View>
                            </View>
                    ...
                    <View style={s.section}>
                        ...
                        <View style={s.infoCard}>
                            <DetailRow icon="person-outline"   label="Full Name" value={applicantName} />
                            <DetailRow icon="mail-outline"     label="Email"     value={applicantEmail} />
                            {application.phone_number ? <DetailRow icon="call-outline"     label="Phone"     value={application.phone_number} /> : null}
                            {application.region       ? <DetailRow icon="location-outline" label="Region"    value={application.region} /> : null}
                            <DetailRow icon="calendar-outline" label="Submitted" value={fmtDateShort(application.submission_date)} />
                        </View>
                    </View>
                            ...
                            {[
                                ...
                                <View key={i} style={s.timelineItem}>
                                    <View style={s.timelineLeft}>
                                        ...
                                        {i < arr.length - 1 && <View style={[s.timelineLine, step.done && s.timelineLineDone]} />}
                                    </View>
                                    <View style={s.timelineRight}>
                                        <Text style={[s.timelineLabel, !step.done && { color: C.muted }]}>{step.label}</Text>
                                        <Text style={s.timelineDate}>{step.done ? fmtDate(step.date) : 'In progress…'}</Text>
                                    </View>
                                </View>
                            ))}
                        </View>
...
const s = StyleSheet.create({
    ...
    errorIcon:     { width: 72, height: 72, borderRadius: 20, backgroundColor: C.roseSoft, justifyContent: 'center', alignItems: 'center', marginBottom: 16 },
    ...
    errBackBtn:    { backgroundColor: C.navy, paddingHorizontal: 24, paddingVertical: 12, borderRadius: 14 },
    errBackBtnText:{ color: '#fff', fontWeight: '700', fontSize: 14 },

    navbar:  { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: C.surface, paddingHorizontal: 16, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: C.border },
    navBack: { width: 40, height: 40, borderRadius: 12, backgroundColor: C.bg, justifyContent: 'center', alignItems: 'center' },
    navTitle:{ fontSize: 16, fontWeight: '700', color: C.text },

    scroll: { flex: 1 },

    statusHero:    { margin: 16, borderRadius: 20, padding: 24, alignItems: 'center' },
    statusIcoWrap: { width: 72, height: 72, borderRadius: 20, justifyContent: 'center', alignItems: 'center', marginBottom: 12 },
    statusLabel:   { fontSize: 22, fontWeight: '800', marginBottom: 4 },
    statusDate:    { fontSize: 13, color: C.muted, marginBottom: 16 },
    cancelBtn:     { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: C.surface, paddingHorizontal: 18, paddingVertical: 11, borderRadius: 14, borderWidth: 1, borderColor: '#FECACA' },
    ...
    pricePill:     { backgroundColor: C.greenSoft, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12, alignItems: 'center' },
    ...
    planPill:      { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: C.bg, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 10, borderWidth: 1, borderColor: C.border },
    planPillText:  { fontSize: 12, color: C.muted, fontWeight: '500' },
    planDetail:    { fontSize: 13, color: C.muted, lineHeight: 20 },

    infoCard:      { backgroundColor: C.surface, borderRadius: 16, paddingHorizontal: 16, borderWidth: 1, borderColor: C.border },

    rejectionCard: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, backgroundColor: C.roseSoft, borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#FECACA' },
