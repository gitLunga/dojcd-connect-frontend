...
export default function MyApplicationsScreen() {
    ...
    const countOf = (f: Filter) => f === 'All' ? applications.length : applications.filter(a => a.application_status === f).length;
                ...
                <View style={s.appInner}>
                    ...
                    <View style={s.pillRow}>
                        ...
                        <View style={[s.pill, {backgroundColor: C.greenSoft}]}>
                            ...
                            <Text
                                style={[s.pillText, {color: C.green, fontWeight: '700'}]}>R{item.monthly_cost}/mo</Text>
                        </View>
                        <View style={s.pill}>
                    ...
                    <View style={s.appFooter}>
                        <Text style={s.appDate}>Applied {new Date(item.submission_date).toLocaleDateString('en-ZA', {
                            day: 'numeric',
                            month: 'short',
                            year: 'numeric'
                        })}</Text>
                        <View style={s.appFooterRight}>
                            {item.application_status === 'Pending' && (
                                <TouchableOpacity
                                    ...
                                    onPress={(e) => {
                                        e.stopPropagation?.();
                                        handleCancel(item.application_id);
                                    }}
                                >
            ...
            <View style={s.topBar}>
                <View style={s.topBarHeader}>
                    <View>
                        ...
                        <Text style={s.pageSub}>{applications.length} total
                            application{applications.length !== 1 ? 's' : ''}</Text>
                    </View>
                            ...
                            >
                                ...
                                {cnt > 0 && (
                                    <View style={[s.filterCount, active && s.filterCountActive]}>
                                        <Text
                                            style={[s.filterCountText, active && s.filterCountTextActive]}>{cnt}</Text>
                                    </View>
                                )}
                            </TouchableOpacity>
                        );
            ...
            <FlatList
                ...
                ListEmptyComponent={
                    <View style={s.empty}>
                        ...
                        <Text
                            style={s.emptyTitle}>{filter === 'All' ? 'No applications yet' : `No ${filter.toLowerCase()} applications`}</Text>
                        <Text
                            style={s.emptySub}>{filter === 'All' ? 'Browse available devices to get started' : `You have no ${filter.toLowerCase()} applications`}</Text>
                        {filter === 'All' && (
